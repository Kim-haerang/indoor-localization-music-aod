...
% 푸리에 변환
TX = fft(txWaveform,nfft);

% 원래 신호에 채널 곱하기
Y = TX.*repmat(H_f(1:nfft,1), 1, NumAntennas);

% 주파수 영역의 노이즈 추가
NO = 1/(sqrt(2.0*enb.CellRefP*double(length(txWaveform)))*SNR);
noise_f = NO*(randn(size(Y)) + 1j*randn(size(Y)));
Y = Y + noise_f;

% 역푸리에 변환
rxWaveform = ifft(Y, nfft);
...
train_input = [I_rx, Q_rx];   % model의 train_data: input data
train_target = data_buf;    % model의 train_data: target data